...
/**
 * Synchronously read a JSON file and parse it into a typed object.
 *
 * @template T - Type of the expected JSON data structure
 *
 * @param path - File system path to the JSON file
 ...
 * ```ts
 * type Config = { port: number }
 * const config = readFileAsJSONSync<Config>("./config.json")
 * console.log(config.port) // Access typed properties
 * ```
 */
const readFileAsJSONSync = <T>(path: string): T => {
  try {
...
export { readFileAsJSON, readFileAsJSONSync }
